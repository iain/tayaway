...
See `TASKS.md` for the full task surface including usage flags. That file is auto-generated by `mise run docs:tasks` from the `mise.toml` descriptions and `usage` specs, so it stays in sync.
