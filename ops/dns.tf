# DNS records this config owns. The apex (`tayaway.nl` A → old VM) is
# deliberately *not* in here — `ovh_domain_zone_record` is per-record, so
# leaving the apex out of state means a `tofu apply` cannot accidentally
# redirect production while the new stack is being commissioned.
#
# The cutover PR (Phase 7) will:
#   1. Add an `ovh_domain_zone_record` for the apex pointing at this VM.
